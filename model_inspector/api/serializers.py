...
from model_inspector.api.dataclasses import (
    Results,
    ResultsListingItem,
    ResultsModelItem,
)
from model_inspector.api.helpers import init_ret
        ...
        if not all:
            ...
            for model in models:
                ...
                if isinstance(items.first(), Collection):
                    for item in items:
                        # if AdminURLFinder().get_edit_url(item): # TODO decide if this is required,
                        # do some testing on real data
                        results.add(ResultsModelItem(request, item).get())
